...
def main(argv=None):
    ...
    if args.command == "doctor":
        ...
        for key in ("TEAM_ID", "CTF_BASE", "CTF_TOKEN", *MODEL_KEYS):
        ...
        if args.phase == "day1":
            ...
            with tempfile.TemporaryDirectory(prefix="incypher-build-") as directory:
                ...
                secret.write_text("".join(f"{k}={shlex.quote(values[k])}\n" for k in MODEL_KEYS),
                                  encoding="utf-8")
        ...
        # Day 2 must never publish an image retaining the Day-1 model credential.
        if args.phase == "day2":
            run(["docker", "run", "--rm", "--network", "none", "--entrypoint", "sh",
                 args.image, "-c", "test ! -e /opt/agent/.day1-llm.env && test ! -e /opt/agent/.validation-id"])
